...
class SettingsManager {
	...
	private function new() {
		var applicationName:String = "Simple web mulitplayer server";
		var baseLaunchCommand:String = "hl P2PServer.hl";

		var commandHelpContent:CLICommandHelpContent = new CLICommandHelpContent(applicationName, baseLaunchCommand);

		commandHelpContent.description = [
			"A server that serves up a simple harness that allows deployment of web clients",
			"that can connect via websockets.",
			"See repo README.MD for details"
		];

		commandHelpContent.examples = [
			"# Set URL of client page",
			"hl P2PServer.hl -url https://mysite.com/client.html",
			""
		];

		cliArgs = new CLIArgHelper(commandHelpContent);
		cliArgs.registerApplicationArgument("url", "frameUrl", "Sets the URL of the content iFrame to load - if blank debug content shown");
		cliArgs.registerApplicationArgument("v", "verboseLogging", "default true - Shows verbose logging");

		this.settings = new SettingsData();
	}

	public function init() {
		cliArgs.parseApplicationArguments(this.settings);
	};
